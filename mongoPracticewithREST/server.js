...
function validatePostCreation(req, res, next) {
    const { title, content, author, country, gender } = req.body;

    if (!title || !content || !author || !country || !gender) {
        return res.status(400).json({ 
            error: "Missing required fields: title, content, author, country, gender" 
        });
    }
